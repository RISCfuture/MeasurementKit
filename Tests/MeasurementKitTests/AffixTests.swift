...
struct AffixTests {
  ...
  /// The precisions the apps this was extracted from actually edit at. The bug that motivated the
  /// API is invisible at whole numbers and appears at every one of the others.
    ...
    for locale in Self.locales {
      for format in Self.lengthFormats {
        for magnitude in [0.0, 1, 2, 42, 3_000] {
          ...
          mismatches.append("\(locale.identifier) at \(magnitude): “\(prefix + number + suffix)” ≠ “\(written)”")
        }
      }
    }
  ...
  /// backwards there.
  @Test("The unit leads the number where the locale writes it first", arguments: ["si_LK", "sw_TZ"])
  func unitLeadsNumber(identifier: String) {
    let length = Measurement(value: 42, unit: UnitLength.feet)

    let (prefix, suffix) = length.affixes(format: Self.lengthFormats[0], in: Locale(identifier: identifier))
