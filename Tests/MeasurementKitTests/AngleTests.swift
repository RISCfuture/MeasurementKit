...
struct AngleTests {
  ...
  func normalizes(input: Double, expected: Double) {
    #expect(degrees(input).normalized.degrees.isApproximatelyEqual(
      to: expected, absoluteTolerance: 1e-9))
  }
  ...
  func signedNormalizes(input: Double, expected: Double) {
    #expect(degrees(input).signedNormalized.degrees.isApproximatelyEqual(
      to: expected, absoluteTolerance: 1e-9))
  }

  /// A negative angle has to be folded twice to arrive in range. The implementation this was
  /// extracted from folded once and answered −20° here, which reads as a course rather than
  /// failing.
  @Test(
  ...
  func reciprocalHandlesNegatives(input: Double, expected: Double) {
    #expect(degrees(input).reciprocal.degrees.isApproximatelyEqual(
      to: expected, absoluteTolerance: 1e-9))
  }
  ...
  func rotates() {
    #expect(degrees(350).rotated(by: degrees(20)).degrees
      .isApproximatelyEqual(to: 10, absoluteTolerance: 1e-9))
    #expect(degrees(10).rotated(by: degrees(-20)).degrees
  ...
  func inverseTrigonometryAnswersAnAngle() {
    #expect(Measurement<UnitAngle>.arcsin(0.5).converted(to: .degrees).value
      .isApproximatelyEqual(to: 30, absoluteTolerance: 1e-9))
    #expect(Measurement<UnitAngle>.arctan(y: 1, x: 1).converted(to: .degrees).value
